...
// Defines the exact way to capture the screen, being: DirectX 11 using the last display available on the first GPU.
...
// Use the 1000x400 zone on that display
...
// Simple iterator to forcibly stop the loop once all targets are hit
int iter = 0;

// Simple way to define how many pixels to skip while looking for the target, found that 8 works reasonably well
int rowInterval = 8;
int colInterval = 8;
while (true)
{
    ...
    // Lock the pixels so they can be accessed and checked, and release once done
    using (captureZone.Lock())
    {
        RefImage<ColorBGRA> image = captureZone.Image;
        
        // Use a double loop to go through every pixel throughout the box, skipping every by every (interval-1) pixels
        for (int i = 0; i < image.Rows.Count / rowInterval; i++)
        {
            for (int j = 0; j < image.Columns.Count / rowInterval; j++)
            {
                // I mostly forget how exactly this works, but if memory serves me well, it works by:
                // If we're inside the Retry box, just stop.
                // But I'm not too sure if it actually works that way lol
                if (i * rowInterval > 695 - 349 && i * rowInterval < 750 - 349 && j * colInterval > 960 - 470 && j * colInterval < 1110 - 470) return;

                // If the pixel we're checking has this exact color, then click it.
                if (image[j * colInterval, i * rowInterval].R == 149 && image[j * colInterval, i * rowInterval].G == 195 && image[j * colInterval, i * rowInterval].B == 232)
                {
                    ...
                    // "Custom" methods to do custom mouse movements, mostly copypasted from pinvoke.net lol
                    MouseOperations.SetCursorPosition((j * colInterval) - 1450, 349 + (i * rowInterval));
                    Thread.Sleep( 50 );
                    MouseOperations.MouseEvent( MouseOperations.MouseEventFlags.LeftDown );
                    MouseOperations.MouseEvent( MouseOperations.MouseEventFlags.LeftUp );

                    // GOTO statement because idk how else to exit all loops except the first one
                    goto thingy;
                }
            }
        }
    ...
    // If we've clicked enough, stop clicking. Usually works, but sometimes I have to rerun the program a few times because it just won't click the right amount of times the first run
thingy:
    {
        if (iter++ > 60) return;
    }
